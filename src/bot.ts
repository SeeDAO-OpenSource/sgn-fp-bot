...
const OPENSEA_PROJECT_NAME = "seedaogenesis";
const FREQUENCY_OF_FETCHING = "3600000";
...
let floorPriceInfo: {
  stats: any; amount: any; currency: any
}
  ...
  while (true) {
    await guilds.map(async (guild: Guild) => {
      ...
      try {
        ...
        // Get new data
        floorPriceInfo = await OpenseaScraper.basicInfo(OPENSEA_PROJECT_NAME)
        // Convert ETH to USD
        floorPriceUSD = floorPriceInfo.stats.floorPrice * perEthToUsd
        // Update bot a new name
        await bot.editSelf(
          { username: `${floorPriceInfo.stats.floorPrice.toFixed(2)} ETH (${floorPriceUSD.toFixed(0)} U)` }
        )
...
async function onReceiveFloorPriceCommand(interaction: CommandInteraction) {
  const message = `${nftProjectName}  ➤  Floor price is ${floorPriceInfo.stats.floorPrice} ETH (${floorPriceUSD} USD).`
